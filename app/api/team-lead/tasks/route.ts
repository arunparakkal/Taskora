  ...
  try {
    ...
    if (assignee_id) {
      if (assignee_id === user.id) {
        return NextResponse.json(
          { error: "Team leads cannot assign tasks to themselves" },
          { status: 400 }
        );
      }
      ...
      if (assigneeProfile?.role !== "member") {
        return NextResponse.json(
    ...
    return handleApiError(error, { route: "POST /api/team-lead/tasks", requestId });
  }
}
